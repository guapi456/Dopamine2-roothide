#!/usr/bin/env python3
"""Repack the bundled RootHide/Sileo applications with Departure identities.

Bundle IDs, display names, application directories, embedded identity strings,
and maintainer scripts change as one transaction. Debian package IDs remain
upstream-compatible so APT dependencies and upgrades continue to work. Every
modified Mach-O is re-signed before the Debian archive is written.
"""
...
STORE = AppSpec(
    ...
    # Same byte length as the original, allowing safe in-place Mach-O literals.
    new_bundle_id="com.departure.marketapp",
    old_app_name="Sileo",
    new_app_name="Sileo",
...
ROOT_HIDE_CLEAN_RULE_REPLACEMENTS = {
    # Longest first because the shorter official identity is a prefix.
    b"com.opa334.Dopamine-roothide": MAIN_BUNDLE_ID.encode(),
    b"com.opa334.Dopamine.roothide": MAIN_BUNDLE_ID.encode(),
    b"com.opa334.Dopamine": MAIN_BUNDLE_ID.encode(),
    STORE.old_bundle_id.encode(): STORE.new_bundle_id.encode(),
    CLEANER.old_bundle_id.encode(): CLEANER.new_bundle_id.encode(),
}
...
def repack_deb(path: Path, spec: AppSpec, ldid: str | None, skip_sign: bool) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="departure-repack-") as temp_dir:
        ...
        if spec is CLEANER:
            # The manager owns RootHide's filesystem trace-cleaning rules.
            replacements.update(ROOT_HIDE_CLEAN_RULE_REPLACEMENTS)
        if spec.rename_payload:
...
def verify_deb(path: Path, spec: AppSpec) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="departure-verify-") as temp_dir:
        ...
        if spec is CLEANER:
            rules = (app / "varCleanRules.json").read_bytes()
            for expected_id in (MAIN_BUNDLE_ID, STORE.new_bundle_id, CLEANER.new_bundle_id):
                if expected_id.encode() not in rules:
                    raise ValueError(f"RootHide cleaning rules omit {expected_id}")
            for stale_id in (
                b"com.opa334.Dopamine-roothide",
                b"com.opa334.Dopamine.roothide",
                b"com.opa334.Dopamine",
                STORE.old_bundle_id.encode(),
                CLEANER.old_bundle_id.encode(),
            ):
                if stale_id in rules:
                    raise ValueError(f"RootHide cleaning rules retain {stale_id!r}")
